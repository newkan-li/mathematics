...
(Ⅰ) $D$ 的面积；
(Ⅱ) $D$ 绕 $x$ 轴旋转一周所得旋转体的体积。`, a: R`(Ⅰ) $\frac12\ln2$；(Ⅱ) $\frac\pi{12}+\left(\ln2-\frac12\right)\pi$（按原书解析，见下）`, sol: R`【解】(Ⅰ) $x\geqslant0$ 时 $f(x)=\lim\limits_{t\to+\infty}\frac x{1+x^2-e^{tx}}=0$；$x<0$ 时 $f(x)=\frac x{1+x^2}$。故 $f(x)=\begin{cases}0,&x\geqslant0,\\\frac x{1+x^2},&x<0.\end{cases}$ $y=f(x)$ 与 $y=\frac12x$ 的交点为 $\left(-1,-\frac12\right),(0,0)$。
...
(Ⅱ) $x\geqslant0$ 部分绕 $x$ 轴旋转为圆锥，$V_1=\frac13\pi\left(\frac12\right)^2\cdot1=\frac\pi{12}$；$x\leqslant0$ 部分旋转体积按原书解析续算（原书在本页截断，结果为 $\left(\ln2-\frac12\right)\pi$），故 $V=\frac\pi{12}+\left(\ln2-\frac12\right)\pi$。` },
